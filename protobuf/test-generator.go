...
func GenerateTestFile(gen *protogen.Plugin, file *protogen.File, handlerFileName string, handlerAst *ast.File) *protogen.GeneratedFile {
	filename := handlerFileName[:len(handlerFileName)-3] + "_test.go"
	...
	if exist {
		...
		addTestToFile(src, g, file, handlerAst)
...
func addTestToFile(src []byte, g *protogen.GeneratedFile, file *protogen.File, handlerAst *ast.File) *protogen.GeneratedFile {
	...
	MethodLoop:
		for methodIndex, method := range srv.Methods {
			for _, d := range astFile.Decls {
				...
				for _, funcDecl := range handlerAst.Decls {
					if genDecl, ok := funcDecl.(*ast.FuncDecl); ok &&
						isMethod(genDecl, method.GoName, srv.GoName) {
						hasHandler = true
					}
				}
			...
			addTestToFile(buff.Bytes(), g, file, handlerAst)
			return g
...
func generateNewTestFile(g *protogen.GeneratedFile, file *protogen.File) *protogen.GeneratedFile {
	...
	g.P("// YOU CAN EDIT this file, and it won't be overridden")
	g.P()
	g.P("package handler")
	g.P()
	...
	log "log"
	testing "testing"`)
	g.P("pb ", "\"", file.GoPackageName, "/", "proto\"")
	g.P(")")
...
func generateTestFunc(buf *bytes.Buffer, g *protogen.GeneratedFile, srv *protogen.Service, method *protogen.Method) {
	...
	FPrint(buf, g, "client := pb.Get", srv.GoName, "Client()")

	if !method.Desc.IsStreamingClient() {
		FPrint(buf, g, "rsp, err := client.", method.GoName, "(", "context.TODO()", ", &", "pb.", method.Input.GoIdent.GoName, "{")
...
func fillFiled(buf *bytes.Buffer, g *protogen.GeneratedFile, field *protogen.Field) {
	if field.Message != nil {
		FPrint(buf, g, field.GoName, ": &pb.", field.GoName, "{")

		for _, field := range field.Message.Fields {
			if field.Message != nil {
				//fillFiled(buf, g, field)
				//TODO: fix struct field stack overflow
			}
		}
