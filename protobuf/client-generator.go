//https://rotemtam.com/2021/03/22/creating-a-protoc-plugin-to-gen-go-code/

package protobuf

import (
	"google.golang.org/protobuf/compiler/protogen"
)

var (
	osPackage           = protogen.GoImportPath("os")
	grpcPackage         = protogen.GoImportPath("google.golang.org/grpc")
	grpcInsecurePackage = protogen.GoImportPath("google.golang.org/grpc/credentials/insecure")
)

// GenerateClientFile generates Service client with go-kit endpoint
func GenerateClientFile(gen *protogen.Plugin, file *protogen.File, mesh, namespace string, port string) *protogen.GeneratedFile {
	filename := file.GeneratedFilenamePrefix + "_client.pb.go"
	g := gen.NewGeneratedFile(filename, file.GoImportPath)
	g.P("// Code generated by protoc-gen-go-kit-grpc. DO NOT EDIT.")
	g.P()
	g.P("package ", file.GoPackageName)
	g.P()

	//create client
	for _, srv := range file.Services {

		g.P("func GetNew", srv.GoName, "Client() ", srv.GoName, "Client {")
		g.P(` var host string`)
		g.P("if ", osPackage.Ident("Getenv"), "(\"CONTAINER\") != \"\" {")
		g.P("host = \"", getServiceMeshHost(srv.GoName, mesh, namespace), "\"")
		g.P(`
	} else {
		host = "localhost"
	}`)
		g.P(" dial, err := ", grpcPackage.Ident("Dial"), "(host+\":", port, "\",",
			grpcPackage.Ident("WithTransportCredentials"),
			"(", grpcInsecurePackage.Ident("NewCredentials"), "()))")

		g.P(`if err != nil {
		panic(err)
	}
`)
		g.P("return New", srv.GoName, "Client(dial)")
		g.P("}")
	}

	return g
}

func getServiceMeshHost(service, mesh, namespace string) string {
	switch mesh {
	case "traefik-mesh":
		return service + "." + namespace + ".traefik.mesh"
	case "istio":
		return service
	default:
		return service
	}
}
