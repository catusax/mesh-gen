...
func GenerateHandlerFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	filename := filename(file.GeneratedFilenamePrefix) + ".go"
...
func addHandlerToFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	MethodLoop:
		for methodIndex, method := range srv.Methods {
			...
			var src []byte
			if v, ok := files[filepath.Join(GetConfig().Handler, filename(file.GeneratedFilenamePrefix)+".go")]; ok {
				src = v
			} else {
				src, err = os.ReadFile(filepath.Join(GetConfig().Handler, filename(file.GeneratedFilenamePrefix)+".go"))
				if err != nil {
					var tmp = gen.NewGeneratedFile(filename(file.GeneratedFilenamePrefix)+".go", file.GoImportPath)
...
`)
			...
			g := gen.NewGeneratedFile(filename(file.GeneratedFilenamePrefix)+".go", file.GoImportPath)
			...
			files[filepath.Join(GetConfig().Handler, filename(file.GeneratedFilenamePrefix)+".go")] = buff.Bytes()
			addHandlerToFile(gen, file)
			g.Skip()
			return nil
		}
...
// TODO: 插入有问题 panic: runtime error: slice bounds out of range [:3988] with capacity 1792
